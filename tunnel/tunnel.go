	...
	go func() {
		...
		if err != nil {
			...
			if rule == nil {
				...
				_ru = rule.RuleType().String()
				_pay = rule.Payload()
				log.Warnln("[UDP] dial %s (match %s/%s) to %s error: %s", proxy.Name(), _ru, _pay, metadata.RemoteAddress(), err.Error())
			}
			utils.LogWriteToFile(`{"remote": "%s", "rule":"%s", "payload":"%s", "chain":"%s", "size":[0, 0], "time":"%s", "protocol":"UDP", "err":"%s"}`,
				metadata.RemoteAddress(), _ru, _pay, proxy.Name(), utils.NowE8Time(), err.Error())
			return
		}
		...
		case mode == Direct:
			utils.LogWriteToFile(`{"remote": "%s", "rule":"DIRECT", "payload":"", "chain":"DIRECT", "size":[0, 0], "time":"%s", "protocol":"UDP", "err":""}`,
				metadata.RemoteAddress(), utils.NowE8Time())
...
func handleTCPConn(connCtx C.ConnContext) {
	...
	if err != nil {
		...
		if rule == nil {
			...
			_ru = rule.RuleType().String()
			_pay = rule.Payload()
			log.Warnln("[TCP] dial %s (match %s/%s) to %s error: %s", proxy.Name(), _ru, _pay, metadata.RemoteAddress(), err.Error())
		}
		utils.LogWriteToFile(`{"remote": "%s", "rule":"%s", "payload":"%s", "chain":"%s", "size":[0, 0], "time":"%s", "protocol":"TCP", "err":"%s"}`,
			metadata.RemoteAddress(), _ru, _pay, proxy.Name(), utils.NowE8Time(), err.Error())
		return
	}
	...
	case mode == Direct:
		utils.LogWriteToFile(`{"remote": "%s", "rule":"DIRECT", "payload":"", "chain":"DIRECT", "size":[0, 0], "time":"%s", "protocol":"TCP", "err":""}`,
			metadata.RemoteAddress(), utils.NowE8Time())
